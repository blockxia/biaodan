...
//      window.addEventListener('orientationchange',function(){self.getMaxWidth()});
      ...
      this.element.addEventListener('touchmove',self.touchmoveFun,false);
      this.element.addEventListener('touchend',self.touchendFun,false);
    ...
    methods:{
      ...
      touchmoveFun(e){
        ...
        let xPos, rect;
        rect = document.querySelector('#drag')
        xPos = e.touches[0].clientX-rect.offsetLeft-1
        self.currentPos={
          x:xPos
        }
        this.curW=self.currentPos.x
